...
public final class RaceData {
    ...
    public static void createCircuit(Player p, String circuitname) {
        if (!getCircuitSet().contains(circuitname)) {
            ...
            Messages.cmdCircuitCreate.sendMessage(p, circuitname);
        } else {
            Messages.cmdCircuitAlreadyExist.sendMessage(p, circuitname);
        }
        saveConfigFile();
    }
    ...
    public static void deleteCircuit(Player p, String circuitname) {
        for (String key : config.getKeys(false)) {
            if (key.equalsIgnoreCase(circuitname)) {
                ...
                saveConfigFile();
                Messages.cmdCircuitDelete.sendMessage(p, circuitname);
                return;
            }
        }
        Messages.invalidCircuit.sendMessage(p, circuitname);
    }

    public static void listCricuit(Player p) {
        Messages.cmdCircuitList.sendMessage(p);
    }
    ...
    public static void editCircuit(Player p, String circuitname) {
        if (!getCircuitSet().contains(circuitname)) {
            ...
            Messages.cmdCircuitEdit.sendMessage(p, circuitname);
        }
    }
    ...
    public static void renameCircuit(Player p, String circuitname, String newname) {
        if (!getCircuitSet().contains(circuitname)) {
            ...
            Messages.cmdCircuitAlreadyExist.sendMessage(p, newname);
        } else {
            for (World w : Bukkit.getWorlds()) {
                for (Entity e : w.getEntities()) {
                    if (RaceManager.isCustomWitherSkull(e, circuitname))
            ...
            Messages.cmdCircuitRename.sendMessage(p, newname);
        }
    }
    ...
    public static void addRunningRaceLapTime(Player p, String circuitname, double laptime) {
        if (getCircuitSet().contains(circuitname)) {
            ...
            if (laptime < config.getDouble(path, 0)) {
                Messages.raceHighScore.sendMessage(p, new Object[] { circuitname,
    ...
    public static void sendRanking(UUID id, String circuitname) {
        ...
        if (ranking == null && kartranking == null)
            Messages.cmdCircuitRankingNoScoreData.sendMessage(id, circuitname);
        else {
            if (kartranking != null)
                Messages.sendAbsolute(id, kartranking);
            if (ranking != null)
                Messages.sendAbsolute(id, kartranking);
        }
    }
    ...
    public static void sendCircuitInformation(Object adress, String circuitname) {
        if (!getCircuitSet().contains(circuitname)) {
            Messages.invalidCircuit.sendMessage(adress);
        }
        ...
        Messages.tableCircuitInformation.sendMessage(adress, new Object[] { circuitname, flag, numberdata });
    }
    ...
    public static void setPosition(Player p, String circuitname) {
        if (!getCircuitSet().contains(circuitname)) {
            Messages.invalidCircuit.sendMessage(p);
        } else {
            Location l = p.getLocation();
            config.set(circuitname + ".world", l.getWorld().getName());
            config.set(circuitname + ".x", l.getX());
            ...
            config.set(circuitname + ".yaw", l.getYaw());
            config.set(circuitname + ".pitch", l.getPitch());
            Messages.cmdCircuitSetPosition.sendMessage(p, circuitname);
            saveConfigFile();
        }
    }
    ...
    public static void setNumberOfLaps(Player p, String circuitname, int amount) {
        if (!getCircuitSet().contains(circuitname)) {
            ...
            Messages.cmdCircuitSetLap.sendMessage(p, new Object[] { circuitname, amount });
            saveConfigFile();
        }
    }
    ...
    public static void setMinPlayer(Player p, String circuitname, int amount) {
        if (!getCircuitSet().contains(circuitname)) {
            ...
            Messages.cmdCircuitOutOfMaxPlayer.sendMessage(p, new Object[] { circuitname, getMaxPlayer(circuitname) });
        } else {
            config.set(circuitname + ".minplayer", amount);
            Messages.cmdCircuitSetMinPlayer.sendMessage(p, new Object[] { circuitname, amount });
            saveConfigFile();
        }
    }
    ...
    public static void setMaxPlayer(Player p, String circuitname, int amount) {
        if (!getCircuitSet().contains(circuitname)) {
            ...
            Messages.cmdCircuitOutOfMinPlayer.sendMessage(p, new Object[] { circuitname, getMinPlayer(circuitname) });
        } else {
            config.set(circuitname + ".maxplayer", amount);
            Messages.cmdCircuitSetMaxPlayer.sendMessage(p, new Object[] { circuitname, amount });
            saveConfigFile();
        }
    }
    ...
    public static void setMatchingTime(Player p, String circuitname, int second) {
        if (!getCircuitSet().contains(circuitname)) {
            ...
            Messages.cmdCircuitSetMatchingTime.sendMessage(p, new Object[] { circuitname, second });
            saveConfigFile();
        }
    }
    ...
    public static void setMenuTime(Player p, String circuitname, int second) {
        if (!getCircuitSet().contains(circuitname)) {
            ...
            Messages.cmdCircuitSetMenuTime.sendMessage(p, new Object[] { circuitname, second });
            saveConfigFile();
        }
    }
    ...
    public static void setLimitTime(Player p, String circuitname, int second) {
        if (!getCircuitSet().contains(circuitname)) {
            ...
            Messages.cmdCircuitSetLimitTime.sendMessage(p, new Object[] { circuitname, second });
            saveConfigFile();
        }
    }
    ...
    public static void setBroadcastGoalMessage(Player p, String circuitname, boolean flag) {
        if (!getCircuitSet().contains(circuitname)) {
            ...
            Messages.cmdCircuitSetBroadcastGoalMessage.sendMessage(p, new Object[] { circuitname, flag });
            saveConfigFile();
        }
    }
